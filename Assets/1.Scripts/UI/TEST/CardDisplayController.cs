...
public class CardDisplayController
{
    ...
    public void SetData(NewCardDataSO cardData)
    {
        ...
        root.Q<Label>("card-level").text = $"Lv. {cardData.basicInfo.cardLevel}";
        root.Q<Label>("card-attribute").text = cardData.basicInfo.type.ToString();
        root.Q<VisualElement>("card-background").style.backgroundColor = cardData.basicInfo.rarityColor;
...
public class CardDisplayController
{
    ...
    public void SetData(NewCardDataSO cardData)
    {
        ...
        // UXML의 다른 요소들도 여기서 데이터를 채울 수 있습니다.
        // 레벨은 NewCardDataSO에 직접 포함되어 있지 않으므로, CardInstance를 통해 설정해야 합니다.
        // CardDisplayHost에서 Setup(CardInstance) 오버로드를 통해 처리될 수 있습니다.

        root.Q<Label>("card-attribute").text = cardData.basicInfo.type.ToString();
        
        // 희귀도 색상은 UIGraphicsDB에서 가져와야 하므로, CardDisplayHost에서 처리하는 것이 적절합니다.
...
public class CardDisplayController
{
    ...
    public void SetData(NewCardDataSO cardData)
    {
        ...
        // 예: root.Q<Label>("card-attribute").text = cardData.basicInfo.type.ToString();
    ...
    // 카드 이름의 폰트 크기를 조절하는 함수입니다. (한 줄 유지, 넘치면 축소)
    ...
    // 카드 설명의 폰트 크기를 조절하는 함수입니다. (여러 줄, 넘치면 축소)
