// 경로: ./TTttTT/Assets/1/Scripts/Data/CardEffects/ApplyEffectToCasterModule.cs
using UnityEngine;
using System.Collections.Generic;
using UnityEngine.AddressableAssets;

/// <summary>
/// [신규] 오직 시전자 자신(Caster)에게만 모든 종류의 효과(스탯, 지속/즉발 피해 및 회복, VFX)를 적용하는 통합 모듈입니다.
/// </summary>
...
public class ApplyEffectToCasterModule : CardEffectSO
{
    ...
    [Header("[ 효과 내용 - 공통 ]")]
    [Tooltip("이 효과를 식별할 고유 ID를 지정합니다. (예: 신속, 과열, 흡혈)")]
    public string StatusEffectID;

    [Tooltip("스탯 변경 및 지속 효과의 지속 시간(초). 0보다 큰 값을 반드시 입력해야 합니다.")]
    public float Duration = 5.0f;

    [Tooltip("동일 효과 중첩 방식을 결정합니다.")]
    public StackingBehavior StackingBehavior = StackingBehavior.RefreshDuration;

    [Header("[ 효과 내용 - 플레이어 스탯 변경 (%) ]")]
    [Tooltip("플레이어의 최종 공격력 보너스를 변경합니다.")]
    public float FinalDamageBonus;
    [Tooltip("플레이어의 최종 공격 속도를 변경합니다.")]
    public float FinalAttackSpeedBonus;
    [Tooltip("플레이어의 최종 이동 속도를 변경합니다.")]
    public float FinalMoveSpeedBonus;
    [Tooltip("플레이어의 최종 최대 체력을 변경합니다.")]
    public float FinalHealthBonus;
    [Tooltip("플레이어의 최종 치명타 확률을 변경합니다.")]
    public float FinalCritRateBonus;
    [Tooltip("플레이어의 최종 치명타 피해량을 변경합니다.")]
    public float FinalCritDamageBonus;

    [Header("[ 효과 내용 - 지속 피해 (DoT) ]")]
    [Tooltip("피해량 계산 방식 (Flat: 고정 수치, MaxHealthPercentage: 최대 체력 비례)")]
    public DamageType DamageType = DamageType.Flat;
    [Tooltip("초당 지속 피해량. (틱 간격은 1초로 고정)")]
    public float DamageAmount;
    [Tooltip("체크 시, 자신의 FinalDamageBonus 스탯이 DoT 피해량에 영향을 줍니다.")]
    public bool ScalesWithDmgBonus = false;

    [Header("[ 효과 내용 - 회복 ]")]
    [Tooltip("회복이 이루어지는 시간 (초). 0일 경우 즉시 회복됩니다.")]
    public float HealDuration;
    [Tooltip("회복량 계산 방식 (Flat: 고정 수치, MaxHealthPercentage: 최대 체력 비례)")]
    ...
    [Tooltip("효과가 지속되는 동안 플레이어에게 붙어서 계속 재생될 VFX의 어드레서블 주소.")]
