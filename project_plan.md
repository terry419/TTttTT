...
1. 정적 참조의 지양 및 동적 참조의 지향 ⭐️

원칙: 씬이 전환될 때 파괴되었다가 다시 생성되는 객체(예: Player, MonsterSpawner, HUDController 등)는 절대로 인스펙터(Inspector) 창에서 다른 객체를 정적으로 참조해서는 안 된다.

사유: 정적 참조는 씬이 파괴되는 순간 연결이 끊어지며(Missing 상태), 새로 로드된 씬의 객체와 자동으로 재연결되지 않아 모든 참조 오류의 근본 원인이 된다.

구현: 모든 씬 종속적인 객체는 자신의 Start() 또는 Awake() 함수 내에서, 필요한 다른 객체를 스스로 찾아 연결하는 동적 참조 방식을 사용해야 한다.

플레이어 참조: Player 오브젝트는 게임 플레이의 가장 핵심적인 대상이므로, "Player" 태그를 사용한다. Player를 참조해야 하는 모든 객체(MonsterSpawner, Camera Follow 등)는 Start() 시점에 GameObject.FindGameObjectWithTag("Player")를 호출하여 동적으로 참조를 획득한다.

씬(Scene) 단위 관리자 참조: Gameplay 씬처럼 특정 씬에서만 존재하는 관리자(MonsterSpawner, HUDController 등)는 다른 객체가 쉽게 찾을 수 있도록 싱글톤(Singleton) 패턴을 사용한다.

C#

public static MyManager Instance { get; private set; }
void Awake() { Instance = this; }
이를 통해 RoundManager와 같이 씬을 초월하여 존재하는 객체가, 새로 로드된 씬의 관리자에게 MyManager.Instance 코드로 즉시 접근할 수 있도록 보장한다.

2. 객체의 책임 분리

원칙: 객체는 자신의 생명주기와 의존성(Dependencies)을 스스로 책임진다.

사유: RoundManager가 MonsterSpawner의 Player 참조까지 신경 쓰는 등, 한 객체가 다른 객체의 내부 상태를 관리하기 시작하면 구조가 복잡해지고 유사한 문제가 반복해서 발생한다.

구현:

MonsterSpawner는 Player를 찾는 책임을 가진다.

HUDController는 Player의 CharacterStats를 찾아 연결하고, 체력 변화 이벤트를 구독하는 책임을 가진다.

RoundManager는 오직 MonsterSpawner.Instance를 호출하여 스폰 시작/중지 명령만 내릴 뿐, 그 내부 동작 방식에 대해서는 관여하지 않는다.