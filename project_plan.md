...
카드 1,2,3 중 하나에 커서를 두어 Enter를 입력 할 경우 선택이 되었음을 유저가 직관적으로 알 수 있도록 한다
카드 1,2,3 중 무엇도 선택되지 않은 상황이면 획득, 합성은 비활성화 상태로 두고, 이 역시 유저가 직관적으로 활성/비활성화를 알 수 있도록 구현한다.
카드를 선택하면 획득은 활성화 되며, 합성의 경우 선택한 카드와 내 캐릭터의 인벤토리에 합성 조건에 맞는 카드가 있다면 활성화 하도록한다.
